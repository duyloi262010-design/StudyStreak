...
const Sidebar: React.FC<SidebarProps> = ({ activeState, onNavigate, onOpenSettings, lang }) => {
  const menuItems = [
    ...
    { id: AppState.SCHEDULE_MANAGER, label: lang === 'vi' ? 'Thời khóa biểu' : 'Schedule', icon: '📅' },
    { id: AppState.PROGRESS, label: lang === 'vi' ? 'Tiến độ học' : 'Progress', icon: '📈' },
    { id: AppState.LESSON_INPUT, label: lang === 'vi' ? 'Luyện tập' : 'Practice', icon: '📖' },
  ];

  return (
    <aside className="fixed bottom-0 left-0 w-full lg:w-72 lg:h-screen lg:sticky lg:top-0 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 p-2 lg:p-6 flex lg:flex-col z-50 border-t lg:border-t-0 lg:border-r border-slate-200 dark:border-slate-800 transition-all duration-300 shadow-xl lg:shadow-none">
      {/* Logo Area */}
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center lg:space-x-4 px-4 py-3 lg:py-4 rounded-2xl transition-all duration-200 group flex-col lg:flex-row ${
                isActive 
                ? 'bg-[#0ea5e9] text-white shadow-lg shadow-sky-200 dark:shadow-none font-black' 
            ...
            >
              ...
              <span className={`text-[10px] lg:text-sm mt-1 lg:mt-0 uppercase tracking-tight lg:tracking-normal ${isActive ? 'opacity-100' : 'opacity-80 group-hover:opacity-100'}`}>
                {item.label}
              </span>
            </button>
          );
