...
const App: React.FC = () => {
  ...
  return (
    <div className={`${isDarkMode ? 'dark' : ''} h-screen flex flex-col`}>
      <div className="flex-1 flex flex-col lg:flex-row bg-white dark:bg-black text-slate-950 dark:text-white transition-colors duration-300 overflow-hidden relative">
          ...
          {activeProfile && appState !== AppState.ACCOUNT_SELECTOR && appState !== AppState.ONBOARDING && (
            <header className="bg-white/95 dark:bg-black/95 backdrop-blur-xl px-4 md:px-8 py-3 flex items-center justify-between sticky top-0 z-40 border-b border-slate-200 dark:border-slate-800 transition-colors shrink-0">
              ...
              <div className="flex items-center space-x-2 md:space-x-5 ml-auto">
                ...
                <button onClick={toggleTheme} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-900 rounded-full text-xl md:text-2xl">{isDarkMode ? '☀️' : '🌙'}</button>
                <div className="relative" ref={menuRef}>
                  <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-100 border-2 border-slate-200 shadow-md flex items-center justify-center text-base md:text-xl overflow-hidden">
                    {activeProfile.avatar || '👤'}
                  </button>
                  {showProfileMenu && (
                    <div className="absolute right-0 mt-4 w-64 md:w-72 bg-white dark:bg-slate-900 rounded-2xl md:rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 py-3 md:py-4 animate-scaleIn z-50">
                      <div className="px-5 md:px-6 py-4 md:py-5 flex flex-col items-center border-b border-slate-100 dark:border-slate-800 mb-2 text-center">
                        ...
                        <button onClick={() => { setIsEditingProfile(true); setShowProfileMenu(false); }} className="w-full mt-3 md:mt-4 py-2.5 md:py-3 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-slate-950 dark:text-white font-black text-[10px] md:text-xs rounded-xl md:rounded-2xl uppercase tracking-widest">{t.manageProfile}</button>
                      </div>
                      <div className="px-2">
          ...
          <main className={`flex-1 ${showSidebar ? 'pb-20 lg:pb-10' : ''}`}>
            <div className={`container mx-auto px-4 ${showSidebar ? 'mt-4 md:mt-6' : 'mt-0'}`}>
              {appState === AppState.ACCOUNT_SELECTOR && (
                ...
                <div className="flex justify-center mt-8 md:mt-12 pb-10"><button onClick={() => setShowDataModal(true)} className="flex items-center space-x-2 px-5 py-2.5 bg-slate-100 dark:bg-slate-900 rounded-xl text-slate-500 font-black text-[10px] md:text-xs uppercase tracking-widest transition-all"><span>💾</span><span>{t.dataManagement}</span></button></div></>
              )}
              {appState === AppState.ONBOARDING && (
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/95 dark:bg-black/95 flex flex-col items-center justify-center z-[100] animate-fadeIn p-6 text-center">
          <div className="relative mb-8 md:mb-10"><div className="w-20 h-20 md:w-28 md:h-28 border-6 md:border-8 border-slate-100 border-t-green-600 rounded-full animate-spin" /></div>
